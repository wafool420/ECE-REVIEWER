...
exam = Reviewer()
exam_number = random.choice(list(exam.exam_sheet.keys()))

print("ECE REVIEWER (FROM QUIZ 1)\n\n")
while 1:

    exam_number = random.choice(list(exam.exam_sheet.keys()))
    print(exam.exam_sheet.get(exam_number)[0])
    for x in exam.exam_sheet.get(exam_number)[2:]:
        print(x)
    ...
    if choice.upper() in valid_choices:
        if choice.upper() == exam.exam_sheet.get(exam_number)[1][0]:
            print(f"CORRECT! The answer is [{exam.exam_sheet.get(exam_number)[1]}]\n")
        else:
            print(f"Wrong! Answer: [{exam.exam_sheet.get(exam_number)[1]}]\n")
    else:
        print("Invalid Answer Detected. Terminating Program")
        exit(1)